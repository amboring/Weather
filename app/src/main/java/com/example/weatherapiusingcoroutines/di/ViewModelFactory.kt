...
class ViewModelFactory @Inject constructor(private val viewModels: Map<Class<out ViewModel>, @JvmSuppressWildcards Provider<ViewModel>>) :
    ...
    inline fun <reified T : ViewModel> getViewModel(viewModelStoreOwner: ViewModelStoreOwner) =
        ViewModelProvider(viewModelStoreOwner, this)[T::class.java]

    inline fun <reified T : ViewModel> getNavScopeViewModel(fragment: Fragment, navGraphId: Int) =
        ViewModelProvider(
            fragment.findNavController().getViewModelStoreOwner(navGraphId),
            this
        )[T::class.java]
}
